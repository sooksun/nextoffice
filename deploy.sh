...
if [ -n "$DB_URL" ]; then
    # db push creates schema (incl. search_text columns + FULLTEXT indexes);
    # apply-search-index installs the trigram function/triggers + backfill —
    # these live outside Prisma's schema, so they must be re-applied each push.
    docker compose run --rm -e "DATABASE_URL=${DB_URL}" api sh -c "npx --yes prisma db push && node prisma/apply-search-index.js" || {
        echo "  Prisma db push / search-index failed — run manually (ใช้ค่า DATABASE_URL จาก .env.production โดยไม่มีเครื่องหมายคำพูดซ้อน):"
        echo "  docker compose run --rm --env-file .env.production api sh -c \"npx prisma db push && node prisma/apply-search-index.js\""
    }
else
    echo "  WARNING: DATABASE_URL not found in .env.production"
    echo "  Run manually: docker compose run --rm --env-file .env.production api sh -c \"npx prisma db push && node prisma/apply-search-index.js\""
fi
