...
async function getDocs(status?: string) {
  const params = new URLSearchParams({ letterType: "directive" });
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-black text-primary tracking-tight print-title">หนังสือสั่งการ — {activeTab.label}</h1>
            <p className="text-xs text-on-surface-variant no-print">
              ตามระเบียบสำนักนายกรัฐมนตรี ว่าด้วยงานสารบรรณ ข้อ ๑๔ — พบ {toThaiNumerals(docs.length)} รายการ
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2 no-print">
          ...
          <Link href="/outbound/new" className="btn-primary flex items-center gap-1.5">
            <Plus size={14} /> สร้างหนังสือสั่งการ
          </Link>
        </div>
      </div>
      ...
      <div className="flex gap-2 mb-4 no-print">
        {SUB_TABS.map((t) => (
          <Link
            key={t.value}
            href={`/saraban/directive?sub=${t.value}`}
      ...
      <form method="GET" className="flex flex-wrap gap-3 mb-5 no-print">
        ...
        <a href={`/saraban/directive?sub=${sub}`} className="btn-ghost">ล้าง</a>
      </form>
