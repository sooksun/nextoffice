...
const SUB_TABS = [
  ...
  { value: "statement", label: "แถลงการณ์" },
...
async function getDocs(status?: string) {
  const params = new URLSearchParams({ letterType: "pr_letter" });
...
export default async function PrPage({
  ...
  const sub = sp.sub ?? "announcement";
  const docs = await getDocs(sp.status);

  const activeTab = SUB_TABS.find((t) => t.value === sub) ?? SUB_TABS[0];
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-black text-primary tracking-tight print-title">หนังสือประชาสัมพันธ์ — {activeTab.label}</h1>
            <p className="text-xs text-on-surface-variant no-print">
              ตามระเบียบสำนักนายกรัฐมนตรี ว่าด้วยงานสารบรรณ ข้อ ๑๕ — พบ {toThaiNumerals(docs.length)} รายการ
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2 no-print">
          ...
          <Link href="/outbound/new" className="btn-primary flex items-center gap-1.5">
            <Plus size={14} /> สร้างหนังสือประชาสัมพันธ์
          </Link>
        </div>
      </div>
      ...
      <div className="flex gap-2 mb-4 no-print">
        {SUB_TABS.map((t) => (
          <Link
            key={t.value}
            href={`/saraban/pr?sub=${t.value}`}
            className={`px-4 py-2 rounded-xl text-sm font-semibold transition-colors ${
              sub === t.value ? "bg-primary text-on-primary" : "bg-surface-bright text-on-surface-variant hover:text-primary"
            }`}
          >
            {t.label}
          </Link>
      ...
      <form method="GET" className="flex flex-wrap gap-3 mb-5 no-print">
        ...
        <a href={`/saraban/pr?sub=${sub}`} className="btn-ghost">ล้าง</a>
      </form>
