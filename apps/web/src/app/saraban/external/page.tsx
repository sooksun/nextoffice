...
async function getDocs(status?: string) {
  const params = new URLSearchParams({ letterType: "external_letter" });
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-black text-primary tracking-tight print-title">หนังสือภายนอก</h1>
            <p className="text-xs text-on-surface-variant no-print">
              ตามระเบียบสำนักนายกรัฐมนตรี ว่าด้วยงานสารบรรณ ข้อ ๑๑ — พบ {toThaiNumerals(docs.length)} รายการ
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2 no-print">
          ...
          <Link href="/outbound/new" className="btn-primary flex items-center gap-1.5">
            <Plus size={14} /> สร้างหนังสือภายนอก
          </Link>
        </div>
      </div>
      ...
      <form method="GET" className="flex flex-wrap gap-3 mb-5 no-print">
        ...
        <a href="/saraban/external" className="btn-ghost">ล้าง</a>
      </form>
            ...
            {docs.map((d, i) => {
              ...
              return (
                <tr key={d.id} className="border-t border-outline-variant/10 hover:bg-surface-bright/50 transition-colors">
                  ...
                  <td className="px-3 py-2 text-xs text-on-surface-variant max-w-[120px] truncate">
