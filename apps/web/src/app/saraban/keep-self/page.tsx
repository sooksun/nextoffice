...
export default function KeepSelfPage() {
  ...
  useEffect(() => {
    ...
    apiFetch<RegistryDoc[]>(`/outbound/${orgId}/registry?type=keep_self`)
      .then(setDocs)
      .catch(() => setDocs([]))
    ...
    <div className="print-full">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-secondary/10 flex items-center justify-center no-print">
            <Boxes size={20} className="text-secondary" />
          </div>
          <div>
            <h1 className="text-2xl font-black text-primary tracking-tight print-title">
              บัญชีหนังสือครบ ๒๐ ปีที่ขอเก็บเอง
            </h1>
            <p className="text-xs text-on-surface-variant">
              ตามระเบียบสำนักนายกรัฐมนตรี ว่าด้วยงานสารบรรณ ข้อ ๕๘.๓ (แบบที่ ๒๒) — พบ {toThaiNumerals(docs.length)} รายการ
            </p>
          </div>
        </div>
        ...
        <table className="w-full text-sm registry-table">
          ...
          <tbody>
            ...
            {!loading && docs.length === 0 && (
              <tr>
                <td colSpan={6} className="px-4 py-10 text-center text-on-surface-variant">ไม่พบรายการ</td>
              </tr>
            )}
            {docs.map((d, i) => (
              <tr key={d.id} className="border-t border-outline-variant/10 hover:bg-surface-bright/50 transition-colors">
                ...
                <td className="px-3 py-2 text-xs max-w-[280px] truncate">{d.subject || "—"}</td>
