    ...
    <div className="space-y-6">
      ...
      {summary && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <StatCard
            label="งานค้างเกิน deadline"
            value={toThaiNumerals(summary.inbound.overdue)}
            color="text-error"
            highlight={summary.inbound.overdue > 0}
          />
          <StatCard
            label="ทะเบียนรับ/ส่ง"
            value={`${toThaiNumerals(summary.registry.inbound)} / ${toThaiNumerals(summary.registry.outbound)}`}
            color="text-tertiary"
          />
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <CardContent className="space-y-2">
              {Object.entries(summary.inbound.byUrgency).map(([key, count]) => (
                <div key={key} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 bg-surface-mid rounded-full h-2">
                    <div
                      className={clsx(
                        "h-2 rounded-full",
                        key === "most_urgent"
        ...
        {summary && (
          <Card>
            <CardHeader>
              <CardTitle className="text-sm">สถานะหนังสือรับ</CardTitle>
            </CardHeader>
            <CardContent className="space-y-2">
          ...
          <CardContent className="space-y-2">
            {workload.map((w) => (
              <div key={w.userId} className="flex items-center gap-3">
                ...
                <span className={clsx("text-sm font-bold", w.activeCases > 5 ? "text-error" : "text-on-surface")}>
...
function StatCard({ label, value, color, highlight }: {
  ...
  return (
    <Card className={clsx(highlight && "border-error/50")}>
