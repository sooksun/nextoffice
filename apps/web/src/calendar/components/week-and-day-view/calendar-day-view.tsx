...
import { thaiDayShort, formatThaiShort, formatThaiTimeRange } from "@/calendar/thai-locale";
        ...
        <div>
          ...
          <div className="relative z-20 flex border-b">
            ...
            <span className="flex-1 border-l py-2 text-center text-xs font-medium text-muted-foreground">
              {thaiDayShort(selectedDate.getDay())} <span className="font-semibold text-foreground">{selectedDate.getDate()}</span>
            </span>
          </div>
        </div>
                  ...
                  return (
                    <div key={event.id} className="space-y-1.5">
                      ...
                      <div className="flex items-center gap-1.5 text-muted-foreground">
                        ...
                        <span className="text-sm">{formatThaiShort(new Date())}</span>
                      </div>

                      <div className="flex items-center gap-1.5 text-muted-foreground">
                        <Clock className="size-3.5" />
                        <span className="text-sm">
                          {formatThaiTimeRange(parseISO(event.startDate), parseISO(event.endDate))}
                        </span>
                      </div>
                    </div>
                  );
